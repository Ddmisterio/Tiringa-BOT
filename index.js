...
//_TIPO DE USUÁRIO
if (groupAdmins) {prema = 'Ademir🤓'}if (isPremium) {prema = 'Premium😎'} if (isOwner) {prema = 'El Ddmisterio🕴🏽'} if (isUser) {prema = 'Registrado 🥳'} else {var prema = 'Membro comum🗿'}
...
tiringa.sendMessage(from, `https://github.com/Ddmisterio/Tiringa-BOT` , text, {quoted: mek})
break
...
case 'pão':
if (isOwner) return reply('Oi Ddmisterio')
else {
...
case 'Ddmisterio':
case 'Ddmisterio':
await tiringa.sendMessage(from, {displayname: "Ddmisterio🧙‍♂️", vcard: vcard}, MessageType.contact)
break
...
//aanu = await fetchJson(`https://api-tiringa.Ddmisterio.repl.co/api/yta?url=${anu.result.source}`)
...
try {
anu = await fetchJson(`https://api-tiringa.Ddmisterio.repl.co/api/yta?url=${play}`)
...
.on('end', function () {
console.log('Finish')
exec(`webpmux -set exif ${addMetadata('Tiringa-BOT', 'Ddmisterio')} ${ran} -o ${ran}`, async (error) => {
...
fs.unlinkSync(ranp)
if (err) return reply(ptbr.stick())
exec(`webpmux -set exif ${addMetadata('Tiringa-BOT', 'Ddmisterio')} ${ranw} -o ${ranw}`, async (error) => {
if (error) return reply(ptbr.stick())
...
.on('error', function (err) {
console.log(`Error : ${err}`)
exec(`webpmux -set exif ${addMetadata('Tiringa-BOT', 'Ddmisterio')} ${rano} -o ${rano}`, async (error) => {
fs.unlinkSync(media)
reply(ptbr.stick())
})
})
...
case 'clearall':
if (!isOwner) return reply('Só o Ddmisterio pode fazer isso')
...
if (!q.includes('|')) return reply(`Digite da forma correta:\nComando: ${prefix}registrar nome|idade\nExemplo: ${prefix}registrar Ddmisterio|18`)
